	...
	if (cur_dev == NULL)
		...
		if (cur_dev->block_read) {
			...
			return cur_dev->block_read (cur_dev->dev
				...
				__u32	size_req;
				do {
					printf(" size_got %x, buf_offset %x, size_total %x\n", size_got, buf_offset, size_total);
					size_req = ((getsize - size_total) < 0xc800) ? (getsize - size_total) : 0xc800;
					size_got = cur_dev->block_read (cur_dev->dev
					, (startblock+size_total), size_req, (unsigned long *)(bufptr + buf_offset));
					printf("after read\n");
					size_total = size_total + size_got;
					buf_offset = size_total * 0x200;
	
					if(size_got != size_req)
					{
						printf("err!!!!! size_got = %x size_req = %x\n",size_got,size_req);
						return -1;
					}
					}
				while(size_total < getsize);
	
				printf(" startblock %x, size_total %x\n", startblock, size_total);
				return size_total;
